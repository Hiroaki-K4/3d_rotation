...
def optimize_R_matrix(R):
    U, s, Vt = np.linalg.svd(R)
    optimized_R = np.dot(np.dot(U, np.diag([1, 1, np.linalg.det(U @ Vt.T)])), Vt)
    return optimized_R
...
def main():
    ...
    print("R: ", R)
    optimized_R = optimize_R_matrix(R)
    print("optimized_R: ", optimized_R)

    # Check if the estimated R is correct
    points_using_optimized_R = np.dot(optimized_R, ori_norm_arr.T)
    ax.scatter(points_using_optimized_R[0], points_using_optimized_R[1], points_using_optimized_R[2], c='red', alpha=0.4)
